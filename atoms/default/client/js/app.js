...
function makeMap(states, places, boundary) {
	...
	function drawMap() {
		...
		svg.append("image")
	        .attr('id', 'Raster')
	        .attr("xlink:href", '<%= path %>/aus-crop-light.png')
	        .attr("class", "raster")
	    ...
	    svg.append("g")
			...
			svg.append("g")
		    .selectAll("path")
		    .data(topojson.feature(boundary,boundary.objects.basin).features)
		    .enter().append("path")
		        .attr("class", "sa2")
		        .attr("fill", "lightblue")
		        .attr("stroke", "#aad8f1")
...
Promise.all([
	d3.json('<%= path %>/au-states.json'),
	d3.json('<%= path %>/places.json'),
	d3.json('<%= path %>/basin.json'), //<%= path %>/mdb_boundary.json
...
.then((results) =>  {
	...
	imageObj.src = '<%= path %>/aus-crop-light.png'
